...
// Load dependencies.
const BinarySearch = require('./LexBinarySearch');
...
function Dictionary(wordlist) {
    ...
    this._rng_start = function (word) {
        let rng = BinarySearch.closest(this.wordlist, word, { exists: false }, Dictionary.prototype.binarySearchCompare);
        let num_tries = 0;
        let lmt = Math.min(Dictionary.prototype.RNG_START_MAX_TRIES, ( this.wordlist.length - rng))
        while (Dictionary.prototype.splitToken(this.wordlist[rng]).startsWith(word) == false && num_tries < lmt) {
            ++rng;
            ++num_tries
        }
        return num_tries < lmt ? rng : -1;
    }



}
Dictionary.prototype.RNG_START_MAX_TRIES = 32;


Dictionary.prototype.completions = function(word,max = 4){
    let s = this._rng_start(word);
    if (s == -1 ) return [];

    out = [];
    while(this.wordlist[s].startsWith(word) && out.length < 4){
        out.push( Dictionary.prototype.splitToken(  this.wordlist[s] ) );
        ++s;
    }
    return out;
}
Dictionary.prototype.binarySearchCompare = function (arr_item, needle) {
    arr_item = Dictionary.prototype.splitToken(arr_item)

    return arr_item > needle ? 1 : arr_item < needle ? -1 : 0;

}


Dictionary.prototype.splitToken = function (word) {
    var w = String(word).split("/")[0].trim();
    /*
    if (w.indexOf(" ") > 0) {
        return w.split(" ")[0]
    } else {
...
Dictionary.prototype.spellCheck = function (word) {
    ...
    var res = BinarySearch(
        ...
        Dictionary.prototype.binarySearchCompare
    );
    return res >= 0;
};
...
Dictionary.prototype.getSuggestions = function (word, limit, maxDistance) {
    ...
    if (word != null && word.length > 0) {
        ...
        if (limit == null || isNaN(limit) || limit <= 0) limit = 4;
        if (maxDistance == null || isNaN(maxDistance) || maxDistance <= 0) maxDistance = 3;
        if (maxDistance >= word.length) maxDistance = word.length - 1;

        // Search index of closest item.
        var closest = BinarySearch.closest(this.wordlist, word, this.binarySearchCompare);
        ...
        for (var i = 0; i < SuggestRadius; i++) {
            ...
            if (k >= 0 && k < this.wordlist.length) {
                ...
                let word_k = Dictionary.prototype.splitToken(this.wordlist[k].toLowerCase());

                let lres = Levenshtien(word, word_k);
                dist  = lres.steps;
